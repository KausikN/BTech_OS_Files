...
void Priority_NP::AddProcess(int pid, int at, int bt, int priority)
{
...
void Priority_NP::RemoveProcess(int pid)
{
...
void Priority_NP::Evaluate()
{
	if(size_of_queue > 0)
	{
		...
		for(int i=1;i<size_of_queue;i++)
		{
			...
			{
				BT_temp[max_priority_index] = -1;

				CT[max_priority_index] = CT[prev_index] + BT[max_priority_index];
