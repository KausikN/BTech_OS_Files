...
void SJF::AddProcess(int pid, int at, int bt)
{
...
void SJF::RemoveProcess(int pid)
{
	...
	if(size_of_queue > 0)
	{
		...
		for(int i=1;i<size_of_queue;i++)
		{
			...
			for(int j=1;j<size_of_queue;j++)
			{
				...
				{
					if(least_bt_index == -1 || BT[least_bt_index] > BT[j]) least_bt_index = j;
				}
			...
			{
				BT_temp[least_bt_index] = -1;

				CT[least_bt_index] = CT[prev_index] + BT[least_bt_index];
...
int main()
{
	...
	for(int i = 1; i <= sjf.no_of_files; i++)
	{
		int pid = i;
		double at;
		int bt;
