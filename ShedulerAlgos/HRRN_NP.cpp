...
void HRRN_NP::AddProcess(int pid, int at, int bt)
{
...
void HRRN_NP::RemoveProcess(int pid)
{
...
float HRRN_NP::ResponseRatio(int index, int prev_index)
{
	...
	if(size_of_queue > 0)
	{
		...
		for(int i=1;i<size_of_queue;i++)
		{
			...
			for(int j=1;j<size_of_queue;j++)
			{
				...
				{
					if(max_hrrn_index == -1 || ResponseRatio(max_hrrn_index, prev_index) < ResponseRatio(j, prev_index)) max_hrrn_index = j;
				}
			...
			{
				BT_temp[max_hrrn_index] = -1;

				CT[max_hrrn_index] = CT[prev_index] + BT[max_hrrn_index];
