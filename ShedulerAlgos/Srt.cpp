...
void SRT::AddProcess(int pid, int at, int bt)
{
...
void SRT::RemoveProcess(int pid)
{
	...
	if(size_of_queue > 0)
	{
		...
		while(no_completed < size_of_queue)
		{
			...
			for(int j=0;j<size_of_queue;j++)
			{
				...
				{
					if(least_bt_index == -1 || BT_temp[least_bt_index] > BT_temp[j]) least_bt_index = j;
				}
				if(BT_temp[j] > 0) 
				{
			...
			else
			{
				...
				BT_temp[least_bt_index] = BT_temp[least_bt_index] - 1;
				CT[least_bt_index] = cur_time;
				
				if(BT_temp[least_bt_index] == 0)
				{
					TAT[least_bt_index] = CT[least_bt_index] - AT[least_bt_index];
					WT[least_bt_index] = TAT[least_bt_index] - BT[least_bt_index];

					no_completed++;
				}
				prev_index = least_bt_index;
			}
			cout<<"\nCURRENT PID: "<<PID[prev_index]<<", AT: "<<AT[prev_index]<<", BT: "<<BT_temp[prev_index]<<", CT: "<<CT[prev_index]<<endl;
		}
	}
...
int main()
{
	...
	for(int i = 1; i <= srt.no_of_files; i++)
	{
		int pid = i;
		double at;
		int bt;
