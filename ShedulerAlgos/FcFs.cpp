...
void FCFS::AddProcess(int pid, int at, int bt)
{
...
void FCFS::RemoveProcess(int pid)
{
...
void FCFS::Evaluate()
{
	if(size_of_queue > 0)
	{
		...
		for(int i=0;i<size_of_queue;i++) order_of_exe[i] = PID[i];
...
int main()
{
	...
	for(int i = 1; i <= fcfs.no_of_files; i++)
	{
		int pid = i;
		double at;
		int bt;
